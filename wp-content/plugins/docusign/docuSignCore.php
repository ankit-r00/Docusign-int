...
class DocuSignCore
{
  ...
  */
  ...
  {
    $rand_id = rand(1000,9000);
    $var = get_option('docuSign');
    $c_name =$_POST['name'];
    $c_email =$_POST['email'];
    $username = $var['username'];
    $password = $var['password'];
    $recipient_role = $var['role'];
      ...
      if (isset($loginInformation) && count($loginInformation) > 0) {
        ...
        if (isset($loginInformation)) {
          ...
          if (!empty($accountId)) {
            ...
            $templateRole->setRoleName($recipient_role);
            $templateRole->setClientUserId($rand_id);
            ...
            if (!empty($envelop_summary)) {
              ...
              $recipient_view_request->setClientUserId($rand_id);
